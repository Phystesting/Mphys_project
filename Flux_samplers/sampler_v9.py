...
def Flux(x, thetaCore, log_n0, p, log_epsilon_e, log_epsilon_B, log_E0, thetaObs, xi_N, d_L, z, jet_type):
    ...
    Flux = []
    try:
        # Iterate over input times and frequencies directly
        for time, freq in zip(t, nu):
            Flux.append(grb.fluxDensity(time, freq, **Z)[0])
        
        # Convert list to array
        Flux = np.array(Flux)
        
        # Validate that the output size matches the input size
        if Flux.shape[0] != len(t):
            raise ValueError("Output flux size does not match input size.")
    except Exception as e:
        print(f"Error in fluxDensity computation: {e}")
        Flux = np.full_like(t, 1e-300)
    print(Flux)
    return Flux
...
# Define times and frequencies for computation
ta = 1.0e-1 * 86400  # Start time (days to seconds)
tb = 1.0e3 * 86400   # End time (days to seconds)
nuR = 6.0e9          # Radio frequency (Hz)
nuO = 1.0e14         # Optical frequency (Hz)
nuX = 1.0e18         # X-ray frequency (Hz)

# Generate the time and frequency grid
Nt = 100
t_values = np.geomspace(ta, tb, num=Nt)
nu_values = [nuR, nuO, nuX, nuO]

x = [np.tile(t_values, len(nu_values)), np.repeat(nu_values, Nt)]

# Calculate flux
flux_values = Flux(x, thetaCore, log_n0, p, log_epsilon_e, log_epsilon_B, log_E0, thetaObs, xi_N, d_L, z, jet_type)
