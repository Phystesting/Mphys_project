...
def log_likelihood(theta, x, y, err, xi_N, d_L, z):
    # Unpack the parameters
    thetaCore, n0, p, epsilon_e, epsilon_B, E0, thetaObs = theta
...
def log_prior(theta):
    ...
    if (45 < E0 < 57
        ...
        return -np.log(1/np.cos(2*thetaObs))
    return -np.inf
...
def run_optimization(x, y, initial, err, xi_N=1.0, d_L=1.0e26, z=0.01):
    ...
    thetaCore_ml, n0_ml, p_ml, epsilon_e_ml, epsilon_B_ml, E0_ml, thetaObs_ml = soln.x
...
def run_parallel_optimization(x, y, initial, err,processes=4, xi_N=1.0, d_L=1.0e26, z=0.01):
    ...
    thetaCore_ml, n0_ml, p_ml, epsilon_e_ml, epsilon_B_ml, E0_ml,thetaObs_ml = best_solution.x
...
def run_sampling(x, y, initial, err, genfile=0,xi_N=1.0,d_L=1.0e26,z=0.01, steps=100, nwalkers=32, processes=4, filename="./data/test_sample.h5"):
    ndim = len(initial)
