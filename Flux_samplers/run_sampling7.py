...
initial = np.array([0.3,1,2.5,-3,-3,54,0.1])
...
err = [abs(0.05*F),err_time,err_spec]
...
if __name__ == "__main__":
    splr.run_sampling(x,F_noise,initial,err,steps=25000,processes=32,genfile=1,filename='./data/seven.h5')


    file_path = './data/seven.h5'
    ...
    fig.savefig('./graph/seven_steps.png')
    plt.close(fig)
    ...
    fig2.savefig('./graph/seven_contour.png')
    plt.close(fig2)
