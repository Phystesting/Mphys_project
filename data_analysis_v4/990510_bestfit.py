...
#file_path_tophat = f'/data/PROJECTS/2024-25/cjc233/samples_v10/{identifier}_TH{extra}_samples.h5'
...
if not os.path.exists(file_path_gaussian):
    raise FileNotFoundError(f"One or both of the HDF5 files '{file_path_gaussian}' do not exist.")
"""
...
"""
output_file_TH = f"./data/{identifier}/{identifier}_TH{extra}_results.txt"
...
with open(output_file_GA, "w") as f:
    # Process TopHat samples
    for i, label in enumerate(labels_gaussian):
...
#best_tophat = np.array(splr.Flux(x_uniform, **parameter_values_tophat, xi_N=xi_N, d_L=d_L, z=z, jet_type=grb.jet.TopHat))
...
handles = [
    ...
    #plt.Line2D([0], [0], linestyle='--', color='black', label='TopHat Fit'),
    plt.Line2D([0], [0], linestyle='-', color='black', label='Gaussian Fit'),
