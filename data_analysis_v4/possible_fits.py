...
# Path to HDF5 file
file_path = f'/data/PROJECTS/2024-25/cjc233/samples_v10/{identifier}_{fit_type}{extra}_samples.h5'
...
# Print final parameter values for verification
...
time, freq, flux, UB_err, LB_err = np.genfromtxt(
    f'../data_generation_v2/data/{identifier}_data_reduced2.csv', delimiter=',', skip_header=1, unpack=True
)
...
for ind in inds:
    ...
    sample_values = {}
    for param, default_value in default_params.items():
        if param in manual_overrides:  # Apply manual overrides first
            sample_values[param] = manual_overrides[param]
