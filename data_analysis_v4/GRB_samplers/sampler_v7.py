...
def log_likelihood(theta, x, y, err_flux, param_names, fixed_params, xi_N, d_L, z, jet_type):
    ...
    """
    err = np.where(model > y, Ub_err, Lb_err)
    # Calculate the combined error term
    sigma2 = err**2
    print(-0.5 * np.sum(np.abs(np.log(sigma2))+((y - model)**2) / sigma2))
    return -0.5 * np.sum(np.abs(np.log(sigma2))+((y - model)**2) / sigma2)
...
def log_prior(theta, param_names):
    ...
    return -(np.exp((Gamma-5000)/1000)+np.exp(-(Gamma-100)/20))  # discourages Lorentz values above 2000 and below 100 and encorages off axis jet predictions
