...
bounds = [(0, None), (None, None)]  # E0 bounded to be positive, log(f) unbounded


E0 = 40
np.random.seed(42)
nll = lambda *args: -log_likelihood(*args)
initial = np.array([E0,np.log(0.01)]) + 0.1 * np.random.randn(2)
...
epsilon_e = 0.1
epsilon_B = 0.0001
y_ml = ag_py(x,thetaObs,thetaCore,thetaWing,n0,p,epsilon_e,epsilon_B,E0_ml)

print("Maximum likelihood estimates:")
print("E0 = {0:.3f}".format(E0_ml))
print("f = {0:.3f}".format(np.exp(f_ml)))

def log_prior(theta):
    E0, log_f = theta
    if -51 < E0 < 54 and -10.0 < log_f < 1.0:
        return 0.0
    return -np.inf
