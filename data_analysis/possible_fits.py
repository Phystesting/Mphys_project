...
# Import samples
file_path = f'/data/PROJECTS/2024-25/cjc233/{identifier}_samples.h5' 
...
for i, label in enumerate(labels):
    if label in ["n0", "epsilon_e", "epsilon_B", "E0"]:  # Log-scaled parameters
        print(f"{label}: {10**mcmc[i][1]:.3e} +{(10**mcmc[i][1])*q[i][0]:.3e} -{(10**mcmc[i][1])*q[i][1]:.3e}")
...
best = np.array(splr.Flux(x_uniform,thetaCore, n0, p, epsilon_e, epsilon_B, E0,thetaObs, xi_N, d_L, z))
...
for nu_value in unique_freqs:
    ...
    ax.plot(t_best_filtered, best_filtered, '-', label=f'Model {nu_value:.2e} Hz')
...
for ind in inds:
    ...
    for nu_value in unique_freqs:
        ...
        ax.plot(
            t_sample_filtered, y_sample_filtered, alpha=0.1, color="gray"
        )
