...
def ag_py(t,thetaObs,thetaCore,thetaWing,n0,p,epsilon_e,epsilon_B,E0):
	Z = {'jetType':     grb.jet.Gaussian,     # Gaussian jet your discrepancy
		 ...
		 'thetaWing':   thetaCore*thetaWing,    # Outer truncation angle
...
def log_prior(theta):
    E0, thetaObs,thetaCore,thetaWing,n0,p,epsilon_e,epsilon_B = theta
    if 51 < E0 < 54 and 0 < thetaObs < np.pi*0.5 and 0 < thetaCore < 0.7 and 1 < thetaWing < 2 and 0 < n0 < 10**3 and 1.8 < p < 3.0 and 0 < epsilon_e < 1 and 0 < epsilon_B < 1:
        return 0.0
    return -np.inf
...
pos = [53, 0.3,0.05,0.4,1e-3,2.3,0.1,0.0001] + 1e-4 * np.random.randn(32, 8)
